...
public class OrderServiceImp implements IOrderService {
    ...
    public ServerResponse create(Integer userId, Integer shippingId) {
        //step1：参数非空校验
        if (shippingId==null){
            return ServerResponse.createServerResponseBySucess("地址参数不能为空");
        }
        ...
        if(orderItemList==null||orderItemList.size()==0){
            return ServerResponse.createServerResponseBySucess("购物车为空");
        }
        orderTotalPrice=getOrderPrice(orderItemList);
        Order order = create(userId, shippingId,orderTotalPrice);
        if (order == null){
            return ServerResponse.createServerResponseBySucess("订单创建失败");
        }
    ...
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        if (order==null){
            return ServerResponse.createServerResponseBySucess("查询的订单不存在");
        }
        //step3：判断订单状态并取消
        if (order.getStatus()!=Const.OrderStatusEnum.ORDER_UN_PAY.getCode()){

            return ServerResponse.createServerResponseBySucess("订单不能取消");
        }
        //step4：返回结果
        ...
        return ServerResponse.createServerResponseBySucess("订单取消失败");
    }
...
//        通过订单号进行查询订单
            Order order = orderMapper.finfOrderByUserIdAndOrderNo(null,orderNo);
//        判断订单的状态，防止支付宝的重复回调
            if(order == null || order.getStatus() >= Const.OrderStatusEnum.ORDER_PAYED.getCode()){
                return Const.FAIL;
            }
//        对回调的状态进行判断
...
//        对订单信息进行修改提交
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for (Cart cart:cartList){
            ...
            if (product.getStatus()!=Const.ProductStatusEnum.PRODUCT_ONLINE.getCode()){//判断商品是否下架
                return ServerResponse.createServerResponseByError("id为"+product.getId()+"的商品已经下架了");
            }
    ...
    public ServerResponse pay(Integer userId, Long orderNo) {
        ...
        return ServerResponse.createServerResponseBySucess("CG",pay(order));
    }
    ...
    public ServerResponse pay(Order order) {
        ...
        String body = "购买商品"+order.getPayment()+"共20.00元";
       ...
       List<OrderItem> orderItemList= orderItemMapper.findOrderItemByOrderNo(order.getOrderNo());
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl(" http://krnsiu.natappfree.cc/shopping/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                map.put("qrCode" ,PropertiesUtils.readByKey("imagesHost")+"/qr-"+response.getOutTradeNo()+".png");
        ...
        return ServerResponse.createServerResponseByError("shibai");
    }
